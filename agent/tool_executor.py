...
def _apply_native_plan_guard(
    ...
    *,
    ...
    try:
        ...
        effective_args = (
            decision.args if isinstance(decision.args, dict) else function_args
        )
        return (
            effective_args,
            (None if decision.allowed else decision.message),
            decision.code,
        )
    except Exception as exc:
        logger.warning(
            "native Plan Mode guard failed closed for %s: %s", function_name, exc
        )
        return (
            function_args,
...
def _record_native_plan_clarification(agent, function_result: Any) -> bool:
    """Persist the Plan Mode clarification checkpoint after a real answer."""
    try:
        payload = (
        ...
        if not isinstance(payload, dict) or payload.get("error"):
            return False
        answered = bool(payload.get("answers")) or bool(
            str(payload.get("user_response") or "").strip()
        )
        if not answered:
            return False
        ...
        logger.warning("native Plan Mode clarification checkpoint failed: %s", exc)
        return False


def _record_native_plan_artifact(
    agent,
    *,
    function_name: str,
    function_args: dict,
    ...
    """Checkpoint only a successful, policy-approved native plan write."""
    if is_error or function_name not in {"write_file", "patch"}:
        return False
    try:
        from hermes_cli.plan_mode import PlanModeManager, evaluate_plan_tool_call
        ...
        if not decision.allowed or decision.code not in {
            "plan_file_write",
            "plan_file_patch",
        }:
            return False
        ...
        if function_name == "write_file":
            artifact_content = str(function_args.get("content") or "")
        else:
            try:
                from tools.file_tools import _resolve_path_for_task

                resolved = _resolve_path_for_task(
                    str(function_args.get("path") or ""),
                    task_id or "default",
                )
                artifact_content = resolved.read_text(
                    encoding="utf-8",
                    errors="replace",
                )
            except Exception:
                logger.warning(
                    "native Plan Mode could not read the patched plan artifact",
                    exc_info=True,
                )
                return False
        manager.mark_plan_artifact_saved(
            str(function_args.get("path") or ""),
            artifact_content,
        )
        return True
    except Exception as exc:
        logger.warning("native Plan Mode plan-artifact checkpoint failed: %s", exc)
        return False
            ...
            try:
                ...
                while True:
                    ...
                    if deadline is not None and time.monotonic() >= deadline:
                        ...
                        _still_running = [parsed_calls[i][1] for i in timed_out_indices]
                        logger.warning(
    ...
    for i, (
        ...
        if agent._should_emit_quiet_tool_messages():
            ...
            if agent.verbose_logging:
                ...
                response_preview = (
                    _preview_str[: agent.log_prefix_chars] + "..."
...
def execute_tool_calls_sequential(
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        _block_error_type = (
            _plan_block_code if _plan_block_msg is not None else "plugin_block"
        )
        if _ts_scope_block is not None:
        ...
        if _block_msg is not None:
            ...
            def _execute(next_args: dict) -> Any:
                ...
                _record_native_plan_clarification(agent, result)
                return result
            ...
            if agent._should_emit_quiet_tool_messages():
                ...
                preview = (
                    _build_tool_label(function_name, display_args) or function_name
                )
                spinner = KawaiiSpinner(
            ...
            try:
                ...
                tool_duration = time.time() - tool_start_time
                cute_msg = _get_cute_tool_message_impl(
                    function_name, function_args, tool_duration, result=_ce_result
                )
                if spinner:
                    spinner.stop(cute_msg)
                elif agent._should_emit_quiet_tool_messages():
        ...
        messages.append(
            make_tool_result_message(function_name, _tool_content, tool_call.id)
        )
        _flush_session_db_after_tool_progress(
            agent,
            messages,
        ...
        if (
            ...
            if agent.verbose_logging:
                ...
                response_preview = (
                    _fr_str[: agent.log_prefix_chars] + "..."
