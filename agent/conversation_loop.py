...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    """Return False when runtime identity or owner-contract revision is stale."""

    from agent.owner_contract import prompt_matches_active_owner_contract

    if not prompt_matches_active_owner_contract(prompt):
        return False
...
def _prepare_native_plan_turn(
    ...
    approval_prompt = (
        build_plan_execution_prompt(state.approval_id) if state.build_pending else ""
    )
    ...
    while (
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                _should_fallback = is_rate_limited or (
