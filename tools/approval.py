        ...
        while prefix_words < 12:
            ...
            if skip_wrapper_options and lower_word.startswith("-"):
                ...
                skip_next_wrapper_arg = (
                    "=" not in lower_word and option_name in _SUDO_OPTIONS_WITH_ARG
                )
                pos = word_end
                prefix_words += 1
                continue
...
_OWNER_HIGH_IMPACT_DESCRIPTION_TERMS = (
    "delete",
    "drop",
    "truncate",
    "force push",
...
_OWNER_DATABASE_MUTATION_RE = re.compile(
    r"(?:"
    r"\balembic\s+upgrade\b|"
    r"\bprisma\s+migrate\s+deploy\b|"
    r"\bmanage\.py\s+migrate\b|"
    r"\b(?:knex|sequelize)\b[^\n;&|]*\b(?:migrate|db:migrate)\b|"
    r"\b(?:medusa|typeorm)\b[^\n;&|]*\bmigrat(?:e|ion)s?\b|"
    r"\b(?:db:)?(?:migrate|migration)\s+(?:up|run|deploy|apply)\b|"
    r"\bbackfill\b[^\n;&|]*(?:--apply|--execute|--production|\bprod\b)"
    r")",
    re.IGNORECASE,
)
...
def detect_owner_high_impact_command(command: str) -> tuple[bool, str | None]:
    ...
    This policy is dormant on ordinary upstream installations.  The verified
    owner contract activates it, keeping routine coding autonomous while
    preserving an exact-action gate for destructive data/file operations,
    secret/config mutation, and database migrations or backfills.
    """
    try:
    ...
    for variant in _command_detection_variants(owner_policy_command):
        if _OWNER_DATABASE_MUTATION_RE.search(variant):
            return (
                True,
                "owner high-impact gate: database migration or production backfill",
            )
    return False, None
...
def prompt_dangerous_approval(
    ...
    if approval_callback is not None:
        try:
            return approval_callback(
                display_command, display_description, allow_permanent=allow_permanent
            )
        except Exception as e:
    ...
    try:
        ...
        while True:
            print()
            print(
                f"  {t('approval.dangerous_header', description=display_description)}"
            )
...
def _get_cron_approval_mode() -> str:
    ...
    try:
        ...
        config = load_config()
        mode = (
            str(cfg_get(config, "approvals", "cron_mode", default="deny"))
            .lower()
            .strip()
        )
...
def _run_approval_gate(
    *,
    ...
    bypass_in_yolo: bool = True,
    cache_approval: bool = True,
) -> dict:
    ...
    if bypass_in_yolo and (_YOLO_MODE_FROZEN or is_current_session_yolo_enabled()):
        return {"approved": True, "message": None}

    session_key = get_current_session_key()
    if cache_approval and is_approved(session_key, pattern_key):
        return {"approved": True, "message": None}
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("HERMES_CRON_SESSION"):
            ...
            return {
                "approved": False,
                "message": no_human_block_message
                or (
    ...
    if is_gateway or env_var_enabled("HERMES_EXEC_ASK"):
        ...
        if notify_cb is not None:
            ...
            approval_data = {
                ...
                "allow_permanent": cache_approval,
            }
            ...
            if cache_approval and choice == "session":
                approve_session(session_key, pattern_key)
            elif cache_approval and choice == "always":
                approve_session(session_key, pattern_key)
...
def check_dangerous_command(
    command: str, env_type: str, approval_callback=None, has_host_access: bool = False
) -> dict:
...
def _await_gateway_decision(
    session_key: str, notify_cb, approval_data: dict, *, surface: str = "gateway"
) -> dict:
...
def check_all_command_guards(
    ...
    # Verified owner policy: exact high-impact actions retain a one-shot,
    # origin-bound approval even in Don't Ask / YOLO / approvals.mode=off.
    # Ordinary code edits, tests, commits, pushes, deploys, and narrow service
    # operations never enter this branch.
    is_owner_high_impact, owner_high_impact_desc = detect_owner_high_impact_command(
        command
    )
    if is_owner_high_impact:
        return _run_approval_gate(
            pattern_key="owner-high-impact:"
            + hashlib.sha256(command.encode("utf-8")).hexdigest(),
            description=str(owner_high_impact_desc),
            display_target=command,
            approval_callback=approval_callback,
            cron_deny_message=(
                f"BLOCKED: {owner_high_impact_desc}; no owner is present to authorize the exact action."
            ),
            autoapprove_log_prefix="BLOCKED owner high-impact action",
            fail_closed_when_no_human=True,
            no_human_block_message=(
                f"BLOCKED: {owner_high_impact_desc}. This exact action requires an origin-bound owner decision."
            ),
            bypass_in_yolo=False,
            cache_approval=False,
        )
...
def check_execute_code_guard(
    code: str, env_type: str, has_host_access: bool = False
) -> dict:
    ...
    if approval_mode == "smart":
        ...
        if verdict == "approve":
            logger.debug(
                "Smart approval: auto-approved execute_code for session %s", session_key
            )
            return {
                "approved": True,
                "message": None,
