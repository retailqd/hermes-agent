...
class TestBuildContextFilesPrompt:
    ...
    def test_agents_md_top_level_only(self, tmp_path):
        """A nested AGENTS.md outside the active cwd scope is ignored."""
    ...
    def test_agents_md_chain_loads_root_to_active_cwd(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / "AGENTS.md").write_text("Root instructions.")
        sub = tmp_path / "src" / "components"
        sub.mkdir(parents=True)
        (tmp_path / "src" / "AGENTS.md").write_text("Source instructions.")
        (sub / "AGENTS.md").write_text("Component instructions.")

        result = build_context_files_prompt(cwd=str(sub))

        assert result.index("Root instructions") < result.index("Source instructions")
        assert result.index("Source instructions") < result.index(
            "Component instructions"
        )

    def test_agents_md_chain_stops_at_git_root(self, tmp_path):
    ...
    def test_agents_md_beats_hermes_md(self, tmp_path):
        """AGENTS.md is the canonical project contract when both exist."""
    ...
    def test_unrelated_legacy_context_does_not_suppress_cursorrules(self, tmp_path):
        (tmp_path / "CLAUDE.md").write_text("Claude guidelines here.")
    ...
    def test_does_not_load_legacy_claude_md(self, tmp_path):
        (tmp_path / "CLAUDE.md").write_text("Use type hints everywhere.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "type hints" not in result
        assert "CLAUDE.md" not in result

    def test_does_not_load_legacy_claude_md_lowercase(self, tmp_path):
        (tmp_path / "claude.md").write_text("Lowercase claude rules.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Lowercase claude rules" not in result

    @pytest.mark.skipif(
        sys.platform == "darwin",
        reason="APFS default volume is case-insensitive; CLAUDE.md and claude.md alias the same path",
    )
    def test_legacy_claude_md_variants_are_both_ignored(self, tmp_path):
    ...
    def test_ignored_legacy_claude_md_is_not_injected(self, tmp_path):
    ...
    def test_agents_md_beats_all_fallback_context(self, tmp_path):
        """When all context types exist, only AGENTS.md is loaded."""
        ...
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Hermes wins" not in result
        assert "Agents lose" in result
